...
echo "$($PSQL "select course from students full join majors using(major_id) full join majors_courses using(major_id) full join courses using(course_id) where student_id is not null and (first_name='Aliya' and last_name='Gulgowski') order by course;")"